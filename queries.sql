...
create view books_with_5_or_less_copies (book_name,noOfCopies) as select *
...
create or replace function no_of_times_books_borrowed_in(int,int)
  returns table (group_id int,no_of_times_borrowed bigint) as $$
    select b.group_id,count(*)
      from register r join books b on b.book_id=r.book_id
...
create or replace function books_borrowed_in(int,int)
  returns table (group_id int,book_name varchar) as $$
    select distinct g.group_id,g.book_name from
      ...
      join book_group g on t1.group_id=g.group_id$$
language sql;
...
create view num_of_times_borrowed as
  select group_id,count(group_id) as noOfTimes
    from books b join register r on r.book_id=b.book_id group by group_id;
...
create view highest_demand_books as
...
create view group_ids_of_books_that_are_in_highest_demand_and_not_available as
  select * from highest_demand_books h where h.group_id not in (select b.group_id from books b
    join highest_demand_books h on b.group_id = h.group_id where b.status= 'available' group by b.group_id);

create view books_that_are_in_highest_demand_and_not_available as
  select bg.book_name from book_group bg
